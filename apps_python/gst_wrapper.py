...
def get_input_str(input):
    ...
    video_dec = {'h264':' ! h264parse ! v4l2h264dec' + \
                 ...
                 'h265':' ! h265parse ! v4l2h265dec' + \
                        ' ! video/x-raw, format=NV12 ', \
                 'auto':' ! decodebin ! tiovxcolorconvert' + \
                        ' ! video/x-raw, format=NV12 '}
    ...
    if (source == 'camera'):
        if input.format == 'jpeg':
            ...
            source_cmd = 'v4l2src device=%s io-mode=5 ! ' % input.source
            source_cmd += 'video/x-bayer, width=%d, height=%d, format=rggb !' % \
                                                     (input.width, input.height)
            #TODO - Take sensor name and subdev as params
            source_cmd += ' tiovxisp device=/dev/v4l-subdev%d' % \
                            input.subdev_id + \
...
def get_output_str(output):
    ...
    cmd = ''
    cmd += 'appsrc format=GST_FORMAT_TIME block=true ' + \
           'num-buffers=1 name=%s ' % output.gst_bkgnd_sink_name
    cmd += '! tiovxcolorconvert '
    cmd += '! video/x-raw,format=NV12, width=%d, height=%d ! queue ' % (output.width, output.height)
    cmd += '! mosaic_%d.background' % output.id + ' \n'

    mosaic_cmd = cmd + 'tiovxmosaic name=mosaic_%d' % output.id + '\n'
...
def get_pre_proc_str(flow):
    ...
    if (float(flow.input.width)/resize[0] > 4 or \
        ...
        cmd += 'video/x-raw, width=%d, height=%d ! tiovxmultiscaler ! ' % \
                                          (max(flow.input.width/4, resize[0]), \
                                           max(flow.input.height/4, resize[1]))
...
def get_post_proc_str(flow):
    ...
    cmd += '! tiovxcolorconvert ! video/x-raw,format=NV12, width=%d, height=%d ! queue ' % (flow.width, flow.height)
    cmd += '! mosaic_%d.sink_%d' % (flow.output.id,flow.disp_id) + ' \n'
